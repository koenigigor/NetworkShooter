...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FCaracterEnhancedWalkForwardTest, "NetworkShooter.Character.Learn.EnhancedWalkForward", EAutomationTestFlags::ApplicationContextMask | EAutomationTestFlags::HighPriority | EAutomationTestFlags::ProductFilter)
...
{
public:
	FPlaySavedInputDataLatentCommand(UWorld* InWorld, UInputComponent* InInputComponent, TArray<FBindingsData> InBindingData, UEnhancedPlayerInput* InEnhancedPlayerInput = nullptr)
...
bool FCaracterEnhancedWalkForwardTest::RunTest(const FString& Parameters)
{
	...
	const int32 MoveForwardIndex = EnhancedInput->GetActionEventBindings().IndexOfByPredicate([](const TUniquePtr<FEnhancedInputActionEventBinding>& ActionBinding)
	{
		return ActionBinding->GetTriggerEvent() == ETriggerEvent::Triggered && ActionBinding->GetAction()->GetName().Equals("IA_MoveForward");
	});

	if (MoveForwardIndex == INDEX_NONE) return false;
